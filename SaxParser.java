...
public class SaxParser extends DefaultHandler  {
	boolean pageTag=false, titleTag=false, textTag=false, categoryFlag=false;
	String titleString;
	...
	public void characters(char[] ch, int start, int length)throws SAXException {
		...
		if(textTag==true){
			...
			if(check!=null){
				...
				parseCategory(textString.toLowerCase(),titleString.toLowerCase());
		}
	}
	...
	public void parseCategory(String textString, String titleString){
		...
		for(int i=0; i<len; i++){
			...
			if(textArray[i]=='['){
				...
				if(count!=0)
					count--;
				if(count==0) {categoryFlag=false; wildCard.setLength(0); str.setLength(0);}
				
			}
			else if(count==2){
				if (textArray[i]>='a'&&textArray[i]<='z'&&categoryFlag==false){
					str.append(textArray[i]);
				}
				else if(str.toString().equalsIgnoreCase("category")){
					categoryFlag=true;
					if (textArray[i]>='0'&&textArray[i]<='9'){
						year.append(textArray[i]);
					}
			...
			if(wildCard.length()!=0){
				...
				if(wildCard.toString().equalsIgnoreCase("births")||wildCard.toString().contains("livingpeople")){
					tm.put(titleString, "Person");
				}
				for(String key : locationWildCards){
					if(wildCard.toString().trim().contains(key.trim()))
	...
	public void endDocument() throws SAXException {
		...
		for(String key: entityOrNot.keySet() ){
			System.out.println(key+":"+entityOrNot.get(key));
		}*/
		try {
			writeToCategoryFile();
		} catch (IOException e) {
